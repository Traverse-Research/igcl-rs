...
"The error indicates that driver cannot get Led state if Led is i2c supported"
                
    }
    ctl_result_t::CTL_RESULT_ERROR_CORE_LED_SET_STATE_NOT_SUPPORTED_FOR_I2C_LED => {
"The error indicates that driver cannot set Led state if Led is i2c supported"
                
    }
    ctl_result_t::CTL_RESULT_ERROR_CORE_LED_TOO_FREQUENT_SET_REQUESTS => {
"The error indicates that Set Led State request is called too frequently too fast"
...
"Invalid Custom VF Curve applied using OverclockWriteCustomVFCurve. Valid VF Curve contains VF Curve Points which are within min/max of gpuVFCurveVoltageLimit, gpuVFCurveFrequencyLimit parameters in ctl_oc_properties_t structure and VFCurve points with distinct volages in ascending order, frequencies in ascending order."
